...
function renderDashboardStats() {
  ...
  const elAriaNetWorth = document.getElementById('aria-net-worth');
  if (elAriaNetWorth) elAriaNetWorth.textContent = sm.formatCurrency(netWorth);
...
function renderPortfolioView() {
  ...
  const riskBadge = document.getElementById('port-risk-badge');
  ...
  if (riskBadge) {
    const cashPct = netWorth > 0 ? (cash / netWorth) * 100 : 100;
    if (cashPct > 50) {
      riskBadge.textContent = '🟢 OPTIMAL LIQUIDITY';
      riskBadge.className = 'badge badge-buy';
    } else if (cashPct > 20) {
      riskBadge.textContent = '🟡 MODERATE EXPOSURE';
      riskBadge.className = 'badge badge-neutral';
    } else {
      riskBadge.textContent = '🔴 HIGH LEVERAGE EXPOSURE';
      riskBadge.className = 'badge badge-sell';
    }
  }
  ...
  const gridContainer = document.getElementById('portfolio-allocation-grid');

  const palette = ['#06b6d4', '#a855f7', '#10b981', '#f59e0b', '#ec4899', '#3b82f6', '#8b5cf6'];
  const cashPct = netWorth > 0 ? (cash / netWorth) * 100 : 100;

  // 1. Render Multi-Segment Gradient Bar
  if (barContainer) {
    let barsHTML = `
      <div class="allocation-bar-segment" style="width: ${cashPct}%; background: linear-gradient(90deg, #475569, #64748b);" title="Free Cash: ${cashPct.toFixed(1)}% (${sm.formatCurrency(cash)})">
        ${cashPct > 8 ? `${cashPct.toFixed(0)}% CASH` : ''}
      </div>
    `;
    ...
    holdingsEntries.forEach(([ticker, holding], idx) => {
      ...
      const pct = netWorth > 0 ? Math.max((val / netWorth) * 100, 0) : 0;
      const color = palette[idx % palette.length];

      barsHTML += `
        <div class="allocation-bar-segment" style="width: ${pct}%; background: ${color};" title="${ticker} ${holding.side}: ${pct.toFixed(1)}% (${sm.formatCurrency(val)})">
          ${pct > 7 ? `${ticker} ${pct.toFixed(0)}%` : ''}
        </div>
      `;
    });
  ...
  // 2. Render Modern Allocation Cards Grid
  if (gridContainer) {
    let cardsHTML = `
      <div class="alloc-card-item" style="--alloc-accent: #64748b;">
        <div class="alloc-card-header">
          <div class="alloc-card-title-wrap">
            <div class="logo-box logo-box-md" style="background:rgba(100,116,139,0.2); border-color:#64748b; color:#94a3b8; font-weight:800; font-family:var(--font-mono); font-size:0.75rem;">$</div>
            <div>
              <h4>Free Cash Reserve</h4>
              <span>UNENCUMBERED MARGIN</span>
            </div>
          </div>
          <span class="alloc-pct-tag" style="color:#94a3b8;">${cashPct.toFixed(1)}%</span>
        </div>
        <div class="alloc-card-val">${sm.formatCurrency(cash)}</div>
        <div class="alloc-card-footer">
          <span>Buffer: Liquid</span>
          <button type="button" class="btn-xs-action" onclick="document.getElementById('deposit-modal').classList.add('active');">+ Deposit</button>
        </div>
      </div>
    `;
    ...
    if (holdingsEntries.length === 0) {
      cardsHTML += `
        <div class="alloc-card-item" style="--alloc-accent: var(--accent-purple); grid-column: span 2;">
          <div class="alloc-card-header">
            <div class="alloc-card-title-wrap">
              <span style="font-size:1.2rem;">⚡</span>
              <div>
                <h4>Capital Deployment Ready</h4>
                <span>100% OF FUNDS AVAILABLE FOR QUANT TRADES</span>
              </div>
            </div>
            <span class="badge badge-buy">READY</span>
          </div>
          <p style="font-size:0.8rem; color:var(--text-muted); margin:0.35rem 0 0.75rem 0;">
            No active market risk exposure. Click below or open the Markets tab to deploy capital with 1x-100x leverage.
          </p>
          <div class="alloc-card-footer" style="border:none; padding:0; margin:0;">
            <span style="color:var(--accent-cyan);">Zero liquidation risk currently active</span>
          </div>
        </div>
      `;
    } else {
      holdingsEntries.forEach(([ticker, holding], idx) => {
        ...
        const val = holding.margin + posPnl;
        ...
        cardsHTML += `
          <div class="alloc-card-item" style="--alloc-accent: ${color};">
            <div class="alloc-card-header">
              <div class="alloc-card-title-wrap">
                ${logoHTML}
                <div>
                  <h4>${asset ? asset.name : ticker}</h4>
                  <span>${ticker} &bull; <strong style="color:${holding.side === 'LONG' ? 'var(--accent-green)' : 'var(--accent-red)'}">${holding.side} ${holding.leverage || 1}x</strong></span>
                </div>
              </div>
              <span class="alloc-pct-tag" style="color:${color};">${pct.toFixed(1)}%</span>
            </div>
            <div class="alloc-card-val">${sm.formatCurrency(val)}</div>
            <div class="alloc-card-footer">
              <span class="${isPos ? 'val-positive' : 'val-negative'}">
                ${isPos ? '+' : ''}${sm.formatCurrency(posPnl)} (${isPos ? '+' : ''}${roePct.toFixed(1)}%)
              </span>
              <button type="button" class="btn-xs-action" style="color:var(--accent-red); border-color:rgba(244,63,94,0.3);" onclick="window.closePosition('${ticker}')">Close</button>
            </div>
          </div>
        `;
      });
  ...
  if (tbody) {
    if (holdingsEntries.length === 0) {
      tbody.innerHTML = `<tr><td colspan="9" style="text-align:center; color:var(--text-dim); padding:2.5rem;">No open margin positions. Your account has zero active trades. Click QUICK DEPLOY above or open the Markets tab to trade.</td></tr>`;
    } else {
...
window.openTradeModal = function (ticker, mode, defaultQty = null, defaultLev = 10) {
  const modal = document.getElementById('trade-modal');
  ...
  const qtyInput = document.getElementById('trade-qty');
  const levButtons = modal ? modal.querySelectorAll('.btn-modal-lev') : [];
  const sm = window.stateManager;
  ...
  if (levButtons.length > 0) {
    levButtons.forEach(b => {
      const isMatch = parseInt(b.getAttribute('data-lev'), 10) === defaultLev;
  ...
  qtyInput.value = defaultQty !== null ? defaultQty : '';
  modal.classList.add('active');
  qtyInput.focus();
...
/* ==========================================================================
   18. Integrated Mainframe A.R.I.A View Controller
   ========================================================================== */
function initMainframeView() {
  const video = document.getElementById('kereos-mainframe-video');
  const typewriterText = "Glad you stopped in. Good taste tends to find us. Now, what are we building?";
  const typewriterElem = document.getElementById('kereos-typewriter-text');
  const cursorElem = document.getElementById('kereos-typewriter-cursor');
  const pillsWrapper = document.getElementById('kereos-pills-wrapper');
  const copyBtn = document.getElementById('kereos-copy-email-btn');

  // 1. Mouse & Touch Scrub Controlled Video
  if (video) {
    let prevX = null;
    let targetTime = 0;
    let isSeeking = false;
    let hasQueuedSeek = false;
    ...
    window.addEventListener('mousemove', (e) => {
      const pageMainframe = document.getElementById('page-mainframe');
      if (!pageMainframe || !pageMainframe.classList.contains('active')) return;
      ...
      targetTime = Math.min(Math.max(0, targetTime + timeOffset), video.duration);
      seekVideo();
    });
    ...
    setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (charIndex < typewriterText.length) {
          typewriterElem.textContent += typewriterText.charAt(charIndex);
          charIndex++;
        } else {
  ...
  // 3. Action Pill Buttons Animation (400ms delay)
  if (pillsWrapper) {
    setTimeout(() => {
      pillsWrapper.classList.add('pills-active');
    }, 400);
  }

  // 4. Copy Email Handler for hanzalnabi5@gmail.com
  if (copyBtn) {
    copyBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const email = 'hanzalnabi5@gmail.com';
      try {
...
/* ==========================================================================
   19. Dedicated A.R.I.A Modal Handlers & Direct Email Dispatch
   ========================================================================== */
window.openAriaModal = function(type) {
  window.closeAriaModals();
  const modalId = `modal-aria-${type}`;
...
window.closeAriaModals = function() {
  document.querySelectorAll('#modal-aria-pitch, #modal-aria-careers, #modal-aria-hello, #modal-aria-operate').forEach(m => {
...
document.addEventListener('click', (e) => {
  if (e.target.classList && e.target.classList.contains('modal-overlay') && e.target.id && e.target.id.startsWith('modal-aria-')) {
    window.closeAriaModals();
  }
});

// Pitch an Idea Submission
window.submitAriaPitch = function(e) {
  e.preventDefault();
  const name = document.getElementById('pitch-name').value;
  const email = document.getElementById('pitch-email').value;
  const cat = document.getElementById('pitch-category').value;
  const details = document.getElementById('pitch-details').value;

  if (window.soundFX) window.soundFX.playBuy();
  showToast(`🚀 Idea submitted! Transmitting proposal to hanzalnabi5@gmail.com...`, 'success');

  // Trigger direct mail client compose
  const mailSubject = encodeURIComponent(`[Mainframe Pitch] ${cat} - ${name}`);
  const mailBody = encodeURIComponent(`From: ${name} (${email})\nCategory: ${cat}\n\nProposal Details:\n${details}`);
  window.open(`mailto:hanzalnabi5@gmail.com?subject=${mailSubject}&body=${mailBody}`, '_blank');

  window.closeAriaModals();
  const form = document.getElementById('form-aria-pitch');
  if (form) form.reset();
};

// Careers Application Submission
window.submitAriaCareer = function(e) {
  e.preventDefault();
  ...
  const link = document.getElementById('career-link').value;
  const intro = document.getElementById('career-intro').value;

  if (window.soundFX) window.soundFX.playBuy();
  showToast(`💼 Application submitted to hanzalnabi5@gmail.com! We will review your profile shortly.`, 'success');

  const mailSubject = encodeURIComponent(`[Job Application] ${role} - ${name}`);
  const mailBody = encodeURIComponent(`Candidate: ${name}\nEmail: ${email}\nRole: ${role}\nPortfolio/GitHub: ${link}\n\nIntroduction:\n${intro}`);
  window.open(`mailto:hanzalnabi5@gmail.com?subject=${mailSubject}&body=${mailBody}`, '_blank');

  window.closeAriaModals();
  const form = document.getElementById('form-aria-career');
  if (form) form.reset();
};

// Hello Message Presets
window.setHelloPreset = function(presetText) {
  const msgInput = document.getElementById('hello-message');
  if (msgInput) {
    msgInput.value = presetText;
    msgInput.focus();
  }
  if (window.soundFX) window.soundFX.playClick();
};

// Hello Message Submission
window.submitAriaHello = function(e) {
  e.preventDefault();
  const email = document.getElementById('hello-sender-email').value;
  const message = document.getElementById('hello-message').value;

  if (window.soundFX) window.soundFX.playClick();
  showToast(`👋 Greeting dispatched to hanzalnabi5@gmail.com! Thank you for connecting with us.`, 'success');

  const mailSubject = encodeURIComponent(`[Hello to A.R.I.A] Message from ${email}`);
  const mailBody = encodeURIComponent(`Visitor: ${email}\n\nMessage:\n${message}`);
  window.open(`mailto:hanzalnabi5@gmail.com?subject=${mailSubject}&body=${mailBody}`, '_blank');

  window.closeAriaModals();
  const form = document.getElementById('form-aria-hello');
  if (form) form.reset();
};

// Quick Portfolio Capital Allocation Handler
window.quickOpenTrade = function(ticker, side, allocPct) {
  const sm = window.stateManager;
  if (!sm) return;
  ...
  if (cash <= 10) {
    ...
    showToast('Please add trading capital first to allocate positions', 'info');
    return;
  }
  ...
  const lev = 10;
  const targetMargin = cash * allocPct;
  const notional = targetMargin * lev;
  const qty = parseFloat((notional / asset.price).toFixed(4));

  window.openTradeModal(ticker, side, qty, lev);
  if (window.soundFX) window.soundFX.playClick();
  showToast(`⚡ Prepared ${Math.round(allocPct * 100)}% allocation into ${ticker} (${qty} units @ ${lev}x)`, 'info');
};
