      ...
      orders.forEach((order, ordIdx) => {
        ...
        if (mappedIdx < candles.length) {
          ...
          if (isLong) {
            ...
            ctx.shadowBlur = 10;
            // Live Pulse Ring around Marker
            const markerPulse = (Math.sin(this.pulsePhase * 2) + 1) / 2;
            ctx.save();
            ctx.strokeStyle = `rgba(16, 185, 129, ${0.4 * (1 - markerPulse)})`;
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.arc(markerX, markerY, 8 + markerPulse * 8, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();
            ...
            // Live Pulse Ring around Short Marker
            const markerPulse = (Math.sin(this.pulsePhase * 2) + 1) / 2;
            ctx.save();
            ctx.strokeStyle = `rgba(244, 63, 94, ${0.4 * (1 - markerPulse)})`;
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.arc(markerX, markerY, 8 + markerPulse * 8, 0, Math.PI * 2);
            ctx.stroke();
